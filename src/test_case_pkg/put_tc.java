...
import req_repo_pkg.put_req_repo;
...
public class put_tc {
	...
	public static void putExecutor() throws IOException {
		...
		for (int i = 0; i < 5; i++) {
			Response complete_response = put_tc.statusCodeExtractor();
			int responseStatusCode = complete_response.statusCode();
			String responseBody = complete_response.getBody().asString();
			String requestBody = put_req_repo.put_request_tc1();

			if (responseStatusCode == 200) {
				put_tc.RequestResponseValidator(requestBody, responseBody);
				Evidencefilecreator.evidenceCreator("Putrequest" + LocalDate.now() + ".txt", requestBody, responseBody);
				break;
			} else {
				int k = i + 1;
				if (k < 5) {
	...
	public static Response statusCodeExtractor() throws IOException {
		...
		Response response = ResponseExtractor.putResponse(put_req_repo.put_request_tc1());
		return response;
	}
